...
class SupabaseQuizHistoryRepository implements QuizHistoryRepository {
  const SupabaseQuizHistoryRepository(this._client, this._requestExecutor);
  ...
  Future<QuizHistorySnapshot> fetchHistory({int limit = 30}) async {
    ...
    try {
      final response = await _requestExecutor.run<Object?>(
        request: () {
          return _client.rpc('get_my_quiz_history', params: {'p_limit': limit});
        },
      );
      ...
      throw QuizHistoryFailure(error.message);
    } on FormatException {
      throw const QuizHistoryFailure(
        'Format sejarah kuiz daripada '
        'server tidak sah.',
      );
  ...
  Future<int> clearHistory() async {
    try {
      final response = await _requestExecutor.run<Object?>(
        ...
        request: () {
          return _client.rpc('clear_my_quiz_history');
        },
      );
      ...
      throw const QuizHistoryFailure(
        'Semua sejarah kuiz tidak dapat '
        'dipadamkan. Semak sambungan '
        'Internet anda.',
      );
    }
  ...
  Map<String, dynamic> _readResponseMap(Object? response) {
    if (response is! Map) {
      throw const QuizHistoryFailure(
        'Response sejarah kuiz daripada '
        'server tidak sah.',
      );
    }
  ...
  String _mapPostgrestMessage(String originalMessage) {
    ...
    if (message.contains('attempt_id is required')) {
      return 'ID percubaan kuiz '
          'tidak sah.';
    }

    if (message.contains('limit must be')) {
      return 'Had sejarah kuiz '
          'tidak sah.';
    }
